...
NOMINAL_PERFORMANCE = round(NOMINAL_POWER_DEMAND / NOMINAL_FILTERED_WATER_SUPPLY, 4) # Performance: kW demand per m^3 (in kW/m^3) 
NOMINAL_PRODUCTION_RATIO = round(NOMINAL_FILTERED_WATER_SUPPLY / NOMINAL_WATER_DEMAND, 4) # Supply per demand. For example if to produce 1 liter of prepared water the plant needs 1.2 liters of fresh water then the ratio is 1.0/1.2 = 0.83
...
def produce_on_supplied_water():
    global WATER_SUPPLIED, PLANED_WATER_DEMAND, PLANED_FILTERED_WATER_SUPPLY, NOMINAL_WATER_DEMAND, NOMINAL_FILTERED_WATER_SUPPLY, NOMINAL_PRODUCTION_RATIO
    global STATUS_WATER_NOT_RECEIVED

    if WATER_SUPPLIED < PLANED_WATER_DEMAND:
        filtered_water = round(WATER_SUPPLIED * NOMINAL_PRODUCTION_RATIO, 4)
    else:
...
def calculate_demands():
    global PLANED_FILTERED_WATER_SUPPLY, PLANED_WATER_DEMAND, PLANED_POWER_DEMAND, NOMINAL_PERFORMANCE, NOMINAL_WATER_DEMAND 
    global NOMINAL_FILTERED_WATER_SUPPLY, MINIMAL_FILTERED_WATER_SUPPLY, MAXIMAL_FILTERED_WATER_SUPPLY, NOMINAL_PRODUCTION_RATIO
    ...
    if(PLANED_FILTERED_WATER_SUPPLY < MINIMAL_FILTERED_WATER_SUPPLY):
        ...
        PLANED_WATER_DEMAND = round(MAXIMAL_FILTERED_WATER_SUPPLY / NOMINAL_PRODUCTION_RATIO, 4)
        PLANED_POWER_DEMAND = round(MAXIMAL_FILTERED_WATER_SUPPLY * NOMINAL_PERFORMANCE, 4)
        PLANED_FILTERED_WATER_SUPPLY = MAXIMAL_FILTERED_WATER_SUPPLY
    else:
        PLANED_WATER_DEMAND = round(PLANED_FILTERED_WATER_SUPPLY / NOMINAL_PRODUCTION_RATIO, 4)
        PLANED_POWER_DEMAND = round(PLANED_FILTERED_WATER_SUPPLY * NOMINAL_PERFORMANCE, 4)
...
def calculate_outage_risk():
    ...
    if not STATUS_FAILURE:
        ...
        if OVERPRODUCTION_MODE:
            CURRENT_FAILURE_POSIBILITY = round(CURRENT_FAILURE_POSIBILITY + MINIMAL_FAILURE_POSIBILITY_CHANGE, 6)
