...
NOMINAL_PERFORMANCE = round(NOMINAL_POWER_DEMAND / NOMINAL_HYDROGEN_SUPPLY, 4) # Performance: kW demand per kg (in kW/kg) 
NOMINAL_PRODUCTION_RATIO = round(NOMINAL_HYDROGEN_SUPPLY / NOMINAL_FILTERED_WATER_DEMAND, 4) # Supply per demand. For example if to produce 1 kg of hydrogen the plant needs 0.2 liters of filtered water then the ratio is 1.0/0.2 = 5.0
...
def produce_on_supplied_filtered_water():
    global FILTERED_WATER_SUPPLIED, PLANED_FILTERED_WATER_DEMAND, PLANED_HYDROGEN_SUPPLY, NOMINAL_PRODUCTION_RATIO, NOMINAL_FILTERED_WATER_DEMAND, NOMINAL_HYDROGEN_SUPPLY
    global STATUS_FILTERED_WATER_NOT_RECEIVED

    if FILTERED_WATER_SUPPLIED < PLANED_FILTERED_WATER_DEMAND:
        hydrogen = round(FILTERED_WATER_SUPPLIED * NOMINAL_PRODUCTION_RATIO, 4)
    else:
...
def calculate_demands():
    global PLANED_HYDROGEN_SUPPLY, PLANED_FILTERED_WATER_DEMAND, PLANED_POWER_DEMAND, NOMINAL_PRODUCTION_RATIO, NOMINAL_PERFORMANCE, NOMINAL_FILTERED_WATER_DEMAND
    ...
    if(PLANED_HYDROGEN_SUPPLY < MINIMAL_HYDROGEN_SUPPLY):
        ...
        PLANED_FILTERED_WATER_DEMAND = round(MAXIMAL_HYDROGEN_SUPPLY / NOMINAL_PRODUCTION_RATIO, 4)
        PLANED_POWER_DEMAND = round(MAXIMAL_HYDROGEN_SUPPLY * NOMINAL_PERFORMANCE, 4)
        PLANED_HYDROGEN_SUPPLY = MAXIMAL_HYDROGEN_SUPPLY
    else:
        PLANED_FILTERED_WATER_DEMAND = round(PLANED_HYDROGEN_SUPPLY / NOMINAL_PRODUCTION_RATIO, 4)
        PLANED_POWER_DEMAND = round(PLANED_HYDROGEN_SUPPLY * NOMINAL_PERFORMANCE, 4)
